defmodule ClarxCore.Auth.UserTokens.UserToken do
  ...
  """
  def generate_token(sub, typ) when is_binary(sub) and is_binary(typ) do
  ...
  defp valid_uuid?(id) when is_binary(id),
    do: String.match?(id, @uuid_pattern)

  defp valid_uuid?(_id), do: false

  defp valid_type?(typ) when is_atom(typ),
    do: Enum.member?(Ecto.Enum.values(UserToken, :type), typ)

  defp valid_type?(typ) when is_binary(typ),
    do: Enum.member?(Ecto.Enum.dump_values(UserToken, :type), typ)
