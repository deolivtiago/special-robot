defmodule ClarxCore.Auth.UserTokens.UserTokenTest do
  ...
  describe "generate_token/2 returns ok" do
    test "when sub and typ are valid" do
  ...
  describe "validate_token/2" do
    test "returns ok when token and typ are valid" do
      tokens =
        @token_types
    ...
    test "returns error when token or typ are invalid" do
      ...
      assert {:error, _reason} = UserToken.validate_token(1, "refresh")
    end
  end
end
