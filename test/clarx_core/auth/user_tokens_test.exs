defmodule ClarxCore.Auth.UserTokensTest do
  ...
  describe "insert_user_token/1 returns ok" do
    test "when the user attributes are valid", %{attrs: attrs} do
      assert {:ok, %UserToken{} = user_token} = UserTokens.insert_user_token(attrs)
  ...
  describe "insert_user_token/1 returns error" do
    test "when the user token attributes are invalid" do
      attrs = %{user_id: "???", token: nil, expiration: 1, type: "invalid"}

      assert {:error, changeset} = UserTokens.insert_user_token(attrs)
      errors = errors_on(changeset)
